...
<button
  {...$$restProps}
  bind:this={element}
  class="WuiButton WuiButton--{variant} WuiButton--{size} WuiButton--{color} WuiButton--gap-{gap} {width ? 'WuiButton--width-' + width : ''} {height ? 'WuiButton--height-' + height : ''} {!$$slots.default ? 'WuiButton--only-icon' : ''} {$$restProps.class || ''}"
...
>
  ...
  {:else}
    {#if $$slots.prefix}
      <slot name="prefix" />
    {:else if prefix}
      <Icon size={size}>{prefix}</Icon>
    {/if}

    {#if $$slots.default}
      <Text bold={bold} size="md" class="WuiButton__text">
        <slot />
      </Text>
    {/if}
